...
exports.login = async (req, res, next) => {
  try {
    const { mobileNumber, fcmToken, timeZone, identifier } = req.body;
    ...
    if (!identifier) {
      throw new CustomError('Identifier (either voter ID or full name) is required', 400);
    }

    // Check if the identifier contains any numbers
    const containsNumbers = /\d/.test(identifier);
    let isVoterId = containsNumbers;
    let voterId = isVoterId ? identifier : null;
    let fullName = !isVoterId ? identifier : null;

    let user;

    if (isVoterId) {
      // If identifier contains numbers, treat it as a voter ID

      // Find a user by voterId
      let userWithVoterId = await User.findByField('voterId', voterId);

      // Handle the case where the voterId is already used by a different user with a phone number
      if (userWithVoterId && userWithVoterId.mobileNumber && userWithVoterId.mobileNumber !== mobileNumber) {
        throw new CustomError('Voter ID already registered with another mobile number', 409);
      }
      ...
      if (userWithVoterId && !userWithVoterId.mobileNumber) {
        ...
        user = userWithVoterId; // Assign the updated user to user for JWT token creation
      } else if (userWithMobileNumber) {
        // Case 2: User already exists with the mobileNumber
        console.log(`User with mobile number: ${mobileNumber} already exists`);
        ...
        // Update the user's details
        user = await User.updateById(userWithMobileNumber.id, {
          fcmToken: fcmToken || userWithMobileNumber.fcmToken,
          timeZone: timeZone || 'UTC',
        });
      } else if (!userWithVoterId) {
        // Case 3: No user exists with this voterId and mobileNumber, create a new user
        console.log(`No user found with voterId: ${voterId} or mobile number: ${mobileNumber}. Creating new user.`);
        ...
        user = userWithVoterId;
      }
    } else {
      // If identifier does not contain numbers, treat it as a full name

      // Find a user by mobileNumber
      let userWithMobileNumber = await User.findByMobileNumber(mobileNumber);

      if (userWithMobileNumber) {
        // If the user exists with the mobile number, update the full name
        console.log(`User with mobile number: ${mobileNumber} already exists`);
        ...
        // Create a new user with full name
        console.log(`No user found with mobile number: ${mobileNumber}. Creating new user with full name.`);
        user = await User.create({
          mobileNumber,
          fullName,
          fcmToken,
...
exports.register = async (req, res, next) => {
  ...
  try {
    ...
    user = await User.updateById(user.id, {
      ...
      epicId: epicId == null ? user.epicId : epicId == voterId ? epicId : user.epicId,
...
exports.getUsers = async (req, res, next) => {
  try {
    ...
    res.status(200).json(response(200, true, 'Epic Users retrieved successfully', result));
  } catch (error) {
    next(error);
  }
};
...
exports.createUser = async (req, res, next) => {
  try {
    ...
    if (existingUser) {
      throw new CustomError('User with this mobile number, voterId or email already exists', 409);
    }
